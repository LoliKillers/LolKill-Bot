...
exports.modStick = (media, lolikiller, loli, from) => {
	out = getRandom('.webp')
	try {
		console.log(media)
		spawn('webpmux', ['-set','exif', './sticker/data.exif', media, '-o', out])
		.on('exit', () => {
			lolikiller.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: loli})
			fs.unlinkSync(out)
			fs.unlinkSync(media)
		})
	} catch (e) {
		console.log(e)
		lolikiller.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: loli })
		fs.unlinkSync(media)
	}
}

exports.modMedia = (media, lolikiller, loli, from, fps) => {
	out = getRandom('.webp')
	try {
		ff(media)
		.on('error', (e) => {
